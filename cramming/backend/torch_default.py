...
from .optimizers import build_muon_with_aux_adamw
...
def _load_optimizer(model, cfg_train, cfg_impl, initial_time):
    ...
    # Special-case combined optimizers that need direct access to the full model.
    if cfg_train.optim.type == "MuonWithAuxAdamW":
        optimizer = build_muon_with_aux_adamw(model, cfg_train, cfg_impl)
    else:
