...
p2=$!
# GPU3: Deeper inner refinement plus carried revision, still last-cycle-gradient.
$RUN env CUDA_VISIBLE_DEVICES="$GPU3" python pretrain.py name="${PREFIX}_trm_h4_l${LO}_ds2_gradlast" seed="$SEED" data="$DATA" train="$TRAIN_CFG" arch="$TRM_ARCH_CFG" budget="$BUDGET" dryrun="$DRYRUN" data.streaming="$DATA_STREAMING" train.batch_size="$TRAIN_BATCH" impl.microbatch_size="$TRAIN_MBS" impl.compile_torch="$COMPILE_TORCH" arch.hidden_size="$HIDDEN" arch.num_attention_heads="$HEADS" arch.num_hidden_layers="$LAYERS" arch.hi_cycles=4 arch.lo_cycles="$LO" arch.deep_supervision_steps=2 arch.inference_steps=2 arch.halt_max_steps=2 arch.grad_last_cycle_only=True arch.embed_factor="$EMBED_FACTOR" arch.expansion="$EXPANSION" arch.q_halt_loss_weight="$HALT_LOSS" arch.halt_exploration_prob="$HALT_EXPLORATION_PROB" "wandb.tags=[trm-refine,pretrain,inner-depth,deep2,gradlast,budget${BUDGET},4gpu]" $PRETRAIN_EXTRA &
p3=$!
...
if [[ "$DO_EVAL" == "True" || "$DO_EVAL" == "true" || "$DO_EVAL" == "1" ]]; then
  ...
  p2=$!
  # GPU3: Eval deeper inner refinement plus carried revision.
  $RUN env CUDA_VISIBLE_DEVICES="$GPU3" python eval.py name="${PREFIX}_trm_h4_l${LO}_ds2_gradlast" seed="$SEED" eval="$EVAL_CFG" eval.checkpoint=latest eval.epochs="$EVAL_EPOCHS" eval.batch_size="$EVAL_BATCH" eval.optim.lr="$EVAL_LR" dryrun="$DRYRUN" impl.microbatch_size="$EVAL_MBS" impl.shuffle_in_dataloader=True impl.compile_torch=False +eval.arch_modifications.deep_supervision_steps=2 +eval.arch_modifications.inference_steps=2 +eval.arch_modifications.halt_max_steps=2 +eval.arch_modifications.grad_last_cycle_only=True +eval.arch_modifications.q_halt_loss_weight="$HALT_LOSS" "wandb.tags=[trm-refine,eval,inner-depth,deep2,gradlast,budget${BUDGET},4gpu]" $EVAL_EXTRA &
  p3=$!
